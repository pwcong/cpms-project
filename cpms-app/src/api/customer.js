...
export function getCustomerTianyanchaInfo(keyword) {
  return get(`/api/cpms/baseTianyancha/getDataBaseOrTianyancha/${keyword}`, {}).then((res) => ({
    data: res.data || [],
...
export function getSkeyeDetail(keyword) {
  return get(`/api/cpms/baseTianyancha/getTianyancha817Api/${keyword}`, {}).then((res) => ({
    data: res.data || [],
    totalSize: (res.data || []).length,
  }));
}
...
export function getCheckingRepeat(name) {
  return get(`/api/cpms/cust/selectRepeat?fdCreditCode=${name}`, {}).then((res) => res || {});
}
...
export function postAddressTranslation(query) {
  return post('/api/cpms/baseAddressConfig/sky-eye-check-change-code', query).then((res) => res.data || {});
}
//分页获取反向客户关联列表
...
export function postInverselyCorrelatedCustomer(query) {
  return post('/api/cpms/custSapRelation/inversely-correlated-customer', query).then((res) => res);
}

// 关联失败的客户关联公司
export function postRelationCompany(query) {
  return post('/api/cpms/custSapCustomer/relation-company', query).then((res) => res.data || {});
}
...
export function postSapAddressTransformation(query) {
  return post('/api/cpms/baseAddressConfig/spd-change-sap', query).then((res) => res.data || {});
}
...
export function getLevelChannel(data) {
  return get(`/api/cpms/baseChannelOrg/getChannelOneTwoThreeBySaleCode/${data}`, {}).then((res) => res.data || {});
}

// 获取下一级渠道
export function getNextLevelChannel(data) {
  return get(`/api/cpms/baseChannel/getChannelByChannel/${data}`, {}).then((res) => res.data || {});
}
