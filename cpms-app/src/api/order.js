...
export function getProjectOrPriceOrContractProductList(query) {
  return get('/api/cpms/orderMain/find-product-list', query).then((res) => res.data || []);
}
...
export function getOrderCredit(customerId) {
  return get(`/api/cpms/orderCredit/crm-credit-info/${customerId}`, {}).then((res) => res.data || {});
}

// 订单模块子弹明细表接口
export function getOrderSplitByArkList(id) {
  return get('/api/cpms/orderSplit/get-by-ark-order-no/', {}).then((res) => res.data || {});
}

// 订单模块查看日志接口
export function getOrderFindPushLog(id, type) {
  return get(`/api/cpms/orderMain/find-push-log/${id}/${type}`, {}).then((res) => res.data || {});
}
...
// 根据sku编码获取库存信息
export function getSkuStock(skuCode) {
  return get(`/api/cpms/itf/fz/get-fz-inventory/${skuCode}`, {}).then((res) => ({
...
export function getOrderSplitList(id) {
  return get(`/api/cpms/orderSplit/get-by-order-id?id=${id}`, {}).then((res) => res.data || []);
}
...
export function getCRMCustomerProductLine(customerCode) {
  return get(`/api/cpms/itf/crm/find_productline_by_receivecusid/${customerCode}`, {}).then((res) => res.data || []);
}
...
export function getErrorOrderSplitList(orderId) {
  return get(`/api/cpms/orderMain/find-order-status/${orderId}`, {}).then((res) => res.data || []);
}
