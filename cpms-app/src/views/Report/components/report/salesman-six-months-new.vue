...
const reqMixin = buildListMixin({
  ...
  getData: async (_) => {
    ...
    const value = await api.report
      .getReportTotalTrend({
        loginNameList: loginName,
        startTime: dayjs().subtract(5, 'M').startOf('M').startOf('d').format('YYYY-MM-DD HH:mm:ss'),
        endTime: dayjs().endOf('M').endOf('d').format('YYYY-MM-DD HH:mm:ss'),
      })
      .then((res) => {
        return res;
      });
    const month = (value.clockReportDto.visitTotalTrend || []).map((d) => {
