...
export default {
  ...
  methods: {
      ...
      try {
        if (!!id) {
          ...
          this.formData = {
            ...
            fdEndTime: !!data.fdEndTime ? dayjs(new Date(data.fdEndTime)).format('YYYY-MM-DD') : data.fdEndTime,
          };
        } else {
          ...
          if (businessCenter.fdBusinessCenter && businessCenter.fdBusinessCenterCode) {
            if (businessCenter.fdBusinessCenter.indexOf('商务中心') < 0) {
    ...
    getProjectName(formData) {
      ...
      return `${formData.fdBusinessCenter || '-'}关于${custName || '-'}${mainSite.fdProvinceName || '-'}${
        mainSite.fdCityName || '-'
      }${formData.fdUse || '-'}的项目报备`;
    },
          ...
          case 'edit':
            ...
            const submit = async (otherForm = {}) => {
              if (action === 'add') {
                ...
                await api.business.putProjectForm({
                  ...formData,
                  ...otherForm,
                  fdProjectStatus: formData.fdProjectStatus === 'draft' ? 'todo' : formData.fdProjectStatus,
            ...
            if (!!checkProject) {
              this.$dialog
                .confirm({
                  title: '存在查重信息，是否还要提交?',
                  message: `${checkProject.fdRepeatPrompt}`,
                })
                .then(async () => {
                  this.loading = true;
                  try {
    ...
    checkFormData(formData) {
      ...
      if (projectSites.filter((d) => d.fdIsMain === '1').length !== 1) {
        this.$toast.fail({
          message: '项目所在地校验失败，有且只能有一条主所在地数据',
        });
        return false;
      }
      ...
      if (otherSites.find((d) => d.fdEntireCountry === '1') && otherSites.length > 1) {
        this.$toast.fail({
          message: '项目所在地校验失败，已选择全国，无法再选择非全国的所在地',
        });
        return false;
      }
