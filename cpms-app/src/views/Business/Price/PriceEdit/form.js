...
export default {
  ...
  computed: {
    ...
    fdEstimatedRevenue() {
      return this.formData.priceMessageListAddParams.reduce((p, c) => p + (c.fdTotalAmount || 0), 0);
    },
  },
  methods: {
    handleDetail(id) {
    ...
    async loadProject(id) {
      ...
      const productPriceList = await api.basedata.getProductPrice({
        productCodes: (projectDetailInfo.projectProductListDtoList || []).map((d) => d.fdProductId),
      ...
      this.formData.priceMessageListAddParams = projectProductList.map((d, index) => {
        ...
        const price = {
          ...d,
          id: '',
          fdProductCategoryName: d.fdProductCategoryName === '完全定制化' ? '' : d.fdProductCategoryName,
          fdProductCategoryCode: d.fdProductCategoryName === '完全定制化' ? '' : d.fdProductCategoryCode,
          ...
          // 单台价差
          fdSinglePrice:
            typeof t.productPrice === 'number' ? this.getAmount((d.fdPrice || 0) - (t.productPrice || 0)) : '',
          ...
          fdTotalApplyPriceAssets:
            typeof t.productPrice === 'number'
              ? this.getAmount((d.fdNum || 0) * ((d.fdPrice || 0) - (t.productPrice || 0)))
              : '',
          fdDeliveryPlan: d.fdShippingDate ? dayjs(new Date(d.fdShippingDate)).format('YYYY-MM-DD') : '',
      ...
      this.formData.fdIsRegion =
        projectDetailInfo.fdRegionNum === '0' || projectDetailInfo.fdRegionNum === '' ? '0' : '1';
        ...
        if (!!id) {
          ...
          this.formData = {
            ...
            fdContractDeliveryTime: data.fdContractDeliveryTime
              ? dayjs(new Date(data.fdContractDeliveryTime)).format('YYYY-MM-DD')
              : '',
            fdBeginTime: data.fdBeginTime ? dayjs(new Date(data.fdBeginTime)).format('YYYY-MM-DD') : '',
            fdOrderEndTime: data.fdOrderEndTime ? dayjs(new Date(data.fdOrderEndTime)).format('YYYY-MM-DD') : '',
          };
          ...
          if (this.formData.priceInstallListAddParams[0]) {
            ...
            this.formData.priceInstallListAddParams = [...this.formData.priceInstallListAddParams];
          }
        } else {
          this.formData = {
            ...this.formData,
    ...
    async submitFormData(formData, action) {
      ...
      formData = {
        ...
        priceInstallListAddParams: (formData.priceInstallListAddParams || []).map((d) => ({
          ...d,
          fdAttKey: this.attKey,
      ...
      this.formData.priceMessageListAddParams.map((d) => {
        d.fdCurrentNetSupplyPrice = d.fdCurrentNetSupplyPrice.toFixed(2); //现净供价
        d.fdApplyForNetContribution = d.fdApplyForNetContribution.toFixed(2); //申请净供价
        d.fdActualOrderPrice = d.fdActualOrderPrice.toFixed(2); //实际开单价
        d.fdInstallCost = d.fdInstallCost.toFixed(2); //安装费
        d.fdUpstairsCost = d.fdUpstairsCost.toFixed(2); //上楼费
        d.fdSingleProjectCost = d.fdSingleProjectCost.toFixed(2); //单台项目费 4位小数
      });
      try {
        switch (action) {
          case 'draft':
    ...
    checkFormData(formData) {
      ...
      if (priceMessageListAddParams.length <= 0) {
        this.$toast.fail('价格信息不能为空');
        return false;
      }

      if (!priceMessageListAddParams.every((d) => d.fdQuantity > 0)) {
        this.$toast.fail('价格信息中数量不能小于等于0');
        return false;
      }

      if (!priceMessageListAddParams.every((d) => !!d.fdProductCategoryName)) {
        this.$toast.fail('价格信息中产品类型不能为空');
        return false;
      }

      if (!priceMessageListAddParams.every((d) => !!d.fdProductName)) {
        this.$toast.fail('价格信息中产品型号不能为空');
        return false;
      }

      if (!priceMessageListAddParams.every((d) => !!d.fdDeliveryPlan)) {
        this.$toast.fail('价格信息中交货计划不能为空');
        return false;
      }

      if (!priceMessageListAddParams.every((d) => typeof d.fdCurrentNetSupplyPrice === 'number')) {
        this.$toast.fail('价格信息中现净供价不能为空');
        return false;
      }

      if (!priceMessageListAddParams.every((d) => d.fdApplyForNetContribution <= d.fdCurrentNetSupplyPrice)) {
        this.$toast.fail('价格信息中申请净供价不得高于现进供价');
        return false;
      }
