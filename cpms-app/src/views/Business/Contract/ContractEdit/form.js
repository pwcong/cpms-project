  ...
  methods: {
    ...
    async loadProject(id) {
      ...
      const productLineList = projectProductList.filter(
        (d, i) => projectProductList.findIndex((_d) => d.fdProductLineCode === _d.fdProductLineCode) === i
      );
      ...
      this.formData.contractPriceAddParamLists = projectProductList.map((d) => ({
        ...
        fdTotalAmount: d.fdPrice * d.fdNum || 0,
        fdDeliveryPlan: d.fdShippingDate ? dayjs(d.fdShippingDate).format('YYYY-MM-DD') : '',
      }));
      ...
      try {
        if (!!id) {
          ...
          this.formData = {
            ...
            fdBeginTime: data.fdBeginTime ? dayjs(data.fdBeginTime).format('YYYY-MM-DD HH:mm:ss') : '',
            fdEndTime: data.fdEndTime ? dayjs(data.fdEndTime).format('YYYY-MM-DD HH:mm:ss') : '',
          };
          ...
          if (this.formData.contractInstallAddParamLists[0]) {
            this.formData.contractInstallAddParamLists = [...this.formData.contractInstallAddParamLists];
          }
        } else {
          this.formData = {
            ...this.formData,
    ...
    async submitFormData(formData, action) {
      ...
      formData = {
        ...
        contractInstallAddParamLists: (formData.contractInstallAddParamLists || []).map((d) => ({
          ...d,
        })),
      };
      ...
      try {
        switch (action) {
          case 'draft':
            await api.contract.postContractCreate({
              ...formData,
              mainId: formData.id,
              fdStatus: 'draft',
            });
          ...
          case 'edit':
            if (formData.fdStatus === 'draft') {
              ...
              await api.contract.putContractEdit(formData);
            }
            break;
          default:
            break;
