<template>
  <list-layout class="page-project-query">
    <template v-slot:header="{ className }">
      <van-nav-bar :class="className" title="法人" left-text="返回" left-arrow @click-left="handleCancel" />
    </template>

    <template v-slot:toolbar="{ className }">
      <div :class="className" style="top: 46px">
        <div class="toolbar">
          <van-search v-model="query.fdCorporateName" clearable placeholder="请输入法人名称"> </van-search>
        </div>
      </div>
    </template>
        ...
        <van-list :value="loadingMore" :finished="!hasMore" finished-text="没有更多了" @load="onLoadMore">
          <card-list :data="list">
            <template v-slot="{ item }">
              <corporate-item :value="item" @click="handleDetail" />
            </template>
          </card-list>
        </van-list>
      </van-pull-refresh>
...
import CorporateItem from './item';
...
const listMixin = buildListMixin({
  ...
  getQuery: (_, query) => ({
    fdCorporateName: query.name,
    ...omit(query, ['filter']),
  }),
  getData: (_, query) => {
    return api.basedata.getCorporateList(query);
  },
});
