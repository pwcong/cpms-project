<template>
  <list-layout class="page-project-query">
    <template v-slot:header="{ className }">
      <van-nav-bar :class="className" title="问题列表" left-text="返回" left-arrow @click-left="handleCancel" />
    </template>

    <template v-slot:toolbar="{ className }">
      <div :class="className" style="top: 46px">
        <div class="toolbar">
          <van-search v-model="query.fdUserName" clearable placeholder="请输入提问者姓名"> </van-search>
        ...
        <van-list :value="loadingMore" :finished="!hasMore" finished-text="没有更多了" @load="onLoadMore">
          <card-list :data="list">
            <template v-slot="{ item }">
              <issue-list-item
                :checkActions="['accept', 'close', 'solved', 'setCommon', 'cacleCommon']"
                :value="item"
                @change="onRefresh"
              />
            </template>
...
import IssueFilter, { convertFilter2Query } from '../components/filter/customer';
