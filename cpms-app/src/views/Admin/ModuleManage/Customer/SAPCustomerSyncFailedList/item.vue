<template>
  <div class="project-item">
    <div class="left">
      <div class="desc">
        <span>客户名称: </span>
        <span>{{ tempValue.fdCustName || '--' }}</span>
      </div>
      <div class="desc">
        <span>客户编码: </span>
        <span>{{ tempValue.customerId || '--' }}</span>
      </div>
      <div class="desc">
        <span>社会统一信用编码: </span>
        <span>{{ tempValue.fdUnifiedCode || '--' }}</span>
      </div>
      <div class="desc">
        <span>业务员: </span>
        <span>{{ tempValue.fdUserName || '--' }}</span>
      </div>
      <div class="desc">
        <span>省市信息: </span>
        <span v-if="tempValue.fdProvince || tempValue.fdCity">{{
          `${tempValue.fdProvince}-${tempValue.fdCity}` || '--'
        }}</span>
      </div>
      <div class="desc">
        <span>创建时间: </span>
        <span>{{ formatDateTime(tempValue.createTime) || '--' }}</span>
      </div>
    </div>

    <div class="bottom">
      <div style="text-align: right; margin-top: 10px">
        <van-button size="mini" type="danger" @click="handleClick(tempValue)"> 错误修复 </van-button>
        <van-button size="mini" type="danger" @click="handlerebuild(tempValue)"> 重新生成 </van-button>
      </div>
    </div>
    <!-- 错误修复 -->
    <error-repair
      :sapCustId="sapCustId"
      :fdUnifiedCode="fdUnifiedCode"
      :companyName="companyName"
      :cpmsCustId="cpmsCustId"
      :visible.sync="cancelVisible"
    />
  </div>
</template>

<script>
import api from '@/api';
import ErrorRepair from './error-repair';
import { buildSelectionMixin, formatMixin } from '@/mixins';
import { commonMixin } from '@/views/mixins';

const selectionMixin = buildSelectionMixin({
  valueType: 'OBJECT',
  defaultValue: {},
});
export default {
  mixins: [selectionMixin, formatMixin, commonMixin],
  components: { ErrorRepair },
  name: 'error-repair-item',
  data() {
    return {
      cancelVisible: false,
      sapCustId: '',
      fdUnifiedCode: '',
      companyName: '',
      cpmsCustId: '',
    };
  },
  methods: {
    handleClick(record) {
      this.cancelVisible = true;
      this.sapCustId = record.sapCustId;
      this.companyName = record.companyName;
      this.cpmsCustId = record.cpmsCustId;
    },
    handlerebuild(record) {
      this.$dialog
        .confirm({
          title: '请确认要重新生成?',
        })
        .then(async () => {
          const res = await api.customer.postInverselyCorrelatedCustomer([record.sapCustId]);
          if (res.code === '-1') {
            this.$toast(res.data);
          } else {
            this.$toast.success('重新生成成功');
          }
        })
        .catch(() => {});
    },
  },
};
</script>

<style lang="scss" scoped>
@import '@/assets/scss/common.scss';

.project-item {
  background-color: white;
  font-size: 12px;
  padding: 10px;
  color: #666666;
  position: relative;
  .left {
    .title {
      font-size: 14px;
      color: #333333;
      margin-right: 70px;
    }

    .desc {
      margin-top: 6px;
      margin-right: 70px;
    }
  }
  .right {
    position: absolute;
    top: 10px;
    right: 10px;
  }
}
</style>
