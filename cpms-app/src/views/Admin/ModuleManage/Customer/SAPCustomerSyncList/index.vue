<template>
  <list-layout class="page-project-query">
    <template v-slot:header="{ className }">
      <van-nav-bar :class="className" title="同步成功客户" left-text="返回" left-arrow @click-left="handleCancel" />
    </template>

    <template v-slot:toolbar="{ className }">
      <div :class="className" style="top: 46px; display: flex">
        <div class="toolbar" style="flex: 1">
          <van-search v-model="query.fdSapCustName" clearable placeholder="请输入客户名称搜索"> </van-search>
