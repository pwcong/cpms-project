  ...
  methods: {
      ...
      try {
        if (!!id) {
          ...
          if (this.formData.spersonVisitFeedbackList) {
            ...
            this.formData.spersonVisitFeedbackList.sysAttContractAspectParams = fileList.map((d) => ({
              text: d.fdFileName,
              value: d.id,
              path: d.fdPath,
            }));
    ...
    async submitFormData(formData, action) {
      ...
      try {
        ...
        this.$toast.success({
          message: `表单${action === 'draft' ? '暂存' : '提交'}成功`,
        });
    ...
    checkFormData(formData) {
      ...
      if (fdVisitPlanYear < this.nowYear || (fdVisitPlanYear === this.nowYear && fdVisitPlanWeek < this.nowWeek)) {
        this.$toast.success({
          message: '不允许创建本周之前的拜访计划',
        });
