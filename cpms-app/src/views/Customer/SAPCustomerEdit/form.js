...
export default {
  ...
  methods: {
      ...
      try {
        if (!!id) {
          ...
          this.formData = {
            ...this.formData,
            ...data,
            fdCreateTime: !!data.fdCreateTime ? dayjs(new Date(data.fdCreateTime)).format('YYYY-MM-DD') : '', //创建时间
        ...
        this.formData = {
          ...this.formData,
          fdReqDept: (dept.orgPosDtos[0] || {}).orgName, //申请人部门
          fdDeptCode: (dept.orgPosDtos[0] || {}).orgCode, //申请人部门code
    ...
    async submitFormData(formData, action) {
      ...
      // };
      try {
        if (!!this.id && formData.fdSapStatus !== 'reject' && action !== 'draft' && formData.fdSapStatus !== 'draft') {
          // 编辑的情况下提交
    ...
    checkFormData(formData) {
      ...
      if (!!formData.fdSapMail && !/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(formData.fdSapMail)) {
        this.$toast.fail('客户邮箱格式错误');
        return true;
      }

      if (custSapCompanyListAddParamList.length < 1) {
        this.$toast.fail('公司代码视图不能为空');
        return true;
      }

      if (custSapSaleListAddParamList.length < 1) {
        this.$toast.fail('销售视图不能为空');
        return true;
      }

      if (
        custSapCompanyListAddParamList.filter(
          (d, i) => custSapCompanyListAddParamList.findIndex((_d) => _d.fdCompanySapCode === d.fdCompanySapCode) !== i
        ).length > 0
      ) {
        this.$toast.fail('公司代码不允许重复选择');
        return true;
      }
