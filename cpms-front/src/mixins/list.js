...
export const listMixin = {
  ...
  computed: {
    hasMore() {
      return (
        this.pageSize * this.pageNo < this.totalSize && this.list.length < this.totalSize
      );
    },
  },
};
...
export function buildListMixin(options) {
  ...
  return {
    ...
    created() {
      ...
      this.onLoad = _.debounce(function (query) {
        ...
        setTimeout(async () => {
          try {
            ...
            const { data = [], totalSize = 0 } = await ctx[getDataPropName](
              ctx,
              targetQuery
            );
      ...
      this.onRefresh = _.debounce(function (reload = true) {
        if (ctx.loading) {
          return;
        }
        !!reload && (ctx.pageNo = 1);
