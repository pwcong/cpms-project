...
export default {
  ...
  methods: {
    ...
    handleDeatil(id, type) {
      this.$router.push({
        path: `/Customer/${
          type === 'interd' ? 'InterdCustomerDetail' : 'InvalidCustomerDetail'
        }/${id}`,
        query: {
          type: type,
        },
        ...
        if (!!id) {
          ...
          if (this.source === 'clue') {
            ...
            if (!!fdClientUnifiedCode) {
              const res = await api.customer.getCustomerTianyanchaInfo(
                fdClientUnifiedCode
              );
              this.formData = {
                ...this.formData,
    ...
    getCommonDetail(data) {
      this.formData = {
        ...data,
        fdCompanyRegtime: !!data.fdCompanyRegtime
          ? dayjs(new Date(data.fdCompanyRegtime)).format('YYYY-MM-DD')
          : '',
