  ...
  <list-layout>
    ...
    <template v-slot:main="{ className }">
      <div :class="className">
          ...
          <template slot="operation" slot-scope="text, record">
            <span>
              ...
              <p-button-link
                ...
                v-if="checkButton(record, 'viewOrder')"
              >
                查看订单
              </p-button-link>
              <p-button-link
                @click="handleViewProduct(record)"
                v-if="checkButton(record, 'viewProduct')"
              >
                查看产品
              </p-button-link>
              <p-button-link
        ...
        >
          <order-product-list :visible="orderVisible" readOnly :moduleId="itemValue.id" />
          <span slot="footer">
            <p-button @click="orderVisible = false">取 消</p-button>
          </span>
        </p-modal>
        <product-list-modal
          readOnly
...
import ProductListModal from '../components/modal-product-list';
import OrderProductList from '../SalesOrderDetail/order-product-list';
...
const listMixin = buildListMixin({
  ...
  getData: async (_, query) => {
    let res = await api.order.getOrderList(query);
    console.log('列表', res);
    return res;
  },
});
