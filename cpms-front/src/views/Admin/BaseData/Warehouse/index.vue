      ...
      <block>
        <template v-slot:title>
          <p-breadcrumb>
            ...
            <p-breadcrumb-item>仓库</p-breadcrumb-item>
          </p-breadcrumb>
        </template>
    ...
    <template v-slot:toolbar="{ className }">
      <div :class="className" style="margin-top: 16px">
        <p-switch-form :col="3" :model="query">
          <p-switch-form-item label="仓库">
            <p-input v-model="query.fdWarehouseName" placeholder="请输入" />
          </p-switch-form-item>
          <template slot="button">
            <p-button type="primary" @click="onRefresh"> 搜索 </p-button>
          </template>
        </p-switch-form>
      </div>
    </template>
