...
export function getClockAddressDetail(id) {
  return get(`/api/cpms/spersonClockAddressConfig/${id}`, {}).then(
    (res) => res.data || {}
  );
}
...
export function postClockAddressMobilePage(form) {
  return post('/api/cpms/spersonClockAddressConfig/mobilePage', form).then(
    (res) => res.data || {}
  );
}
...
export function getVisitTypeClockInAddress(query) {
  return get('/api/cpms/spersonVisitMain/visit-address', query).then(
    (res) => res.data || []
  );
}
//获取所有打卡
...
export function getVisitFeedbackDetail(id) {
  return get(`/api/cpms/spersonVisitFeedbackList/${id}`, {}).then(
    (res) => res.data || {}
  );
}
//获取所有的拜访记录
